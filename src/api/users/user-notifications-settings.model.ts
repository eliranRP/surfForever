...
import { Hours, HoursKind, NotificationKind, Rating, WaveTypeId } from './types';
...
class UserNotificationSettingsModel extends BaseCrudModel<IUserNotificationSettingsSchema> {
  ...
  async setPreferredHours(chatId: number, hoursKey: HoursKind) {
    ...
    return await this.upsert({ chatId }, { preferredReminderHours: selectedHours, chatId });
  }
  ...
  async setPreferredWavHeight(chatId: number, waveTypeId: WaveTypeId) {
    ...
    return await this.upsert({ chatId }, { waveHeightRange: selectedWaveHight.height, chatId });
  }
  ...
  private async enrichSpotDetailsById(spotId: string): Promise<SpotLocation> {
    ...
    if (enrichDetails.length == 0) throw new Error(`Location ${spot.spot.name} not found`);
  ...
  async setPreferredNotification(chatId: number, notificationResponse: NotificationKind) {
    const selectedNotification = getNotificationResponseByKind(notificationResponse);
    return await this.upsert(
      { chatId },
  ...
  async notifyUsers(usersSettings: IUserNotificationSettings[]) {
    ...
    return await Promise.all(
      usersSettings.map(async (settings) => {
        const matchForecasts = await checkMatchBetweenForecastAndUserSettings(settings.chatId);
